...
type Registration = { _id: string; registrationCode?: string; registrationStatus: string; checkInStatus: string; checkedInAt?: string; studentId?: { name?: string; email?: string; registerNumber?: string }; eventId?: { title?: string } };
...
const formatDateTime = (value?: string) => value ? new Date(value).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" }) : "-";
...
export default function AdminCheckInPage() {
  ...
  useEffect(() => {
    ...
    if (!key) return;
    Promise.all([
      fetch(`${API_URL}/api/admin/events`, { headers: { "x-admin-key": key } }).then(async (response) => {
        const data = await response.json();
  ...
  useEffect(() => {
    ...
    scanner.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 240, height: 240 } }, (decodedText) => {
      ...
      setScanning(false);
      setError("Camera access is unavailable. Use the registration input instead.");
    });
    return () => {
  ...
  async function checkIn(value: string) {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "Could not check in student");
    } finally {
      setLoading(false);
      processingRef.current = false;
      inputRef.current?.focus();
  ...
  return <main className="admin-layout"><AdminSidebar active="check-in" /><section className="admin-main"><div className="admin-top"><div><div className="eyebrow">Admin workspace</div><h1 className="display">Check-in</h1><p>Scan or enter a student&apos;s registration ID to check them in to an event.</p></div></div><div className="check-in-layout"><form onSubmit={submit}><label className="field"><span>Registration ID</span><input ref={inputRef} className="admin-key-input" value={registrationCode} onChange={(event) => setRegistrationCode(event.target.value)} placeholder="Scan or enter registration ID" autoComplete="off" /></label><label className="field"><span>Checking in for</span><select value={eventId} onChange={(event) => setEventId(event.target.value)}><option value="">Any event</option>{events.map((event) => <option key={event._id} value={event._id}>{event.title} · {formatDate(event.date)}</option>)}</select></label><button className="btn secondary" type="submit" disabled={loading}>{loading ? "Checking in..." : "Check in"}</button></form><button className="btn ghost" onClick={() => setScanning(true)} disabled={scanning}>Start QR Scanner</button>{scanning && <div id="admin-check-in-reader" className="qr-reader" />}{message && registration && <div className="check-in-result success-state"><b>✓ {message}</b><div className="detail-grid"><span>Student name<b>{registration.studentId?.name ?? "-"}</b></span><span>Email<b>{registration.studentId?.email ?? "-"}</b></span><span>Event<b>{registration.eventId?.title ?? "-"}</b></span><span>Registration ID<b>{registration.registrationCode ?? "-"}</b></span><span>Checked in<b>{formatDateTime(registration.checkedInAt)}</b></span></div></div>}{error && <div className="admin-error">{error}</div>}<div className="section-heading"><div><div className="eyebrow">History</div><h2>Recent check-ins</h2></div></div>{checkIns.length === 0 ? <div className="admin-empty">No check-ins yet.</div> : <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Student</th><th>Email</th><th>Event</th><th>Registration</th><th>Time</th><th>Checked in by</th><th>Status</th></tr></thead><tbody>{checkIns.map((checkIn) => <tr key={checkIn._id}><td>{checkIn.studentName}</td><td>{checkIn.studentEmail}</td><td>{checkIn.eventTitle}</td><td>{checkIn.registrationId}</td><td>{formatDateTime(checkIn.checkedInAt)}</td><td>{checkIn.checkedInBy}</td><td>{checkIn.status}</td></tr>)}</tbody></table></div>}</div></section></main>;
}
